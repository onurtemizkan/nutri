...
PERSONAS = [
    PersonaConfig(
        "elite_athlete", 28, 0.95, 0.3, 0.85, 0.9, 52, 3, 75, 15, 6, "active", 0.3
    ),
    PersonaConfig(
        "recreational_runner", 35, 0.7, 0.4, 0.7, 0.75, 62, 4, 55, 12, 4, "active", 0.4
    ),
    PersonaConfig(
        "office_worker", 42, 0.3, 0.7, 0.5, 0.5, 78, 6, 28, 10, 1, "sedentary", 0.6
    ),
    PersonaConfig(
        "stressed_executive",
        48,
    ...
    PersonaConfig(
        "health_enthusiast", 32, 0.65, 0.35, 0.8, 0.85, 65, 4, 50, 10, 5, "active", 0.35
    ),
    PersonaConfig(
        "shift_worker", 38, 0.35, 0.75, 0.35, 0.45, 75, 7, 30, 14, 1.5, "mixed", 0.8
    ),
    PersonaConfig(
        "college_student", 21, 0.5, 0.6, 0.45, 0.4, 70, 5, 45, 18, 2, "mixed", 0.5
    ),
    PersonaConfig(
        "retired_active", 65, 0.55, 0.25, 0.75, 0.7, 68, 5, 35, 10, 4, "active", 0.5
    ),
    PersonaConfig(
        "new_parent", 33, 0.4, 0.8, 0.3, 0.5, 76, 7, 32, 15, 1, "sedentary", 0.6
    ),
    PersonaConfig(
        "yoga_practitioner", 40, 0.6, 0.2, 0.9, 0.8, 60, 3, 58, 8, 5, "active", 0.25
    ),
]
...
class UltraRealisticDataGenerator:
    ...
    def __init__(
        self, personas: List[PersonaConfig], days: int = 730, seq_length: int = 30
    ):
    ...
    def _apply_nutrition_effect(
        self, nutrition: Dict, fitness_level: float
    ) -> Tuple[float, float]:
        """Calculate nutrition impact on RHR and HRV."""
        # Protein positively affects HRV
        protein_effect_hrv = (
            (nutrition["protein"] - 100) / 100 * 5 * (1 - fitness_level * 0.5)
        )
    ...
    def _apply_sleep_effect(
        self, sleep_hours: float, sleep_quality: float
    ) -> Tuple[float, float]:
    ...
    def _apply_exercise_effect(
        self, exercise: Dict, fitness_level: float, days_since: int
    ) -> Tuple[float, float]:
    ...
    def _apply_stress_effect(
        self, stress_level: float, stress_event: bool
    ) -> Tuple[float, float]:
    ...
    def generate_persona_data(self, persona: PersonaConfig) -> pd.DataFrame:
        ...
        for day in range(self.days):
            ...
            seasonal = self._generate_seasonal_pattern(
                day, persona.seasonal_sensitivity
            )
            ...
            sleep_quality_daily = np.clip(
                persona.sleep_quality + np.random.normal(0, 0.15), 0, 1
            )
            ...
            exercise = {
                ...
                "type": (
                    np.random.choice(["cardio", "strength", "mixed"])
                    if is_exercise_day
                    else "none"
                ),
            }
            ...
            # Calculate effects
            nutr_rhr, nutr_hrv = self._apply_nutrition_effect(
                nutrition, persona.fitness_level
            )
            sleep_rhr, sleep_hrv = self._apply_sleep_effect(
                sleep_hours, sleep_quality_daily
            )
            exer_rhr, exer_hrv = self._apply_exercise_effect(
                exercise, persona.fitness_level, days_since_exercise
            )
            stress_rhr, stress_hrv = self._apply_stress_effect(
                persona.stress_level, stress_event
            )

            # Calculate final values with autocorrelation
            rhr = 0.7 * prev_rhr + 0.3 * (
                persona.base_rhr
                + seasonal
                + weekly
                + circadian * persona.rhr_variability
                + nutr_rhr
                + sleep_rhr
                + exer_rhr
                + stress_rhr
    ...
    def create_sequences(
        self, df: pd.DataFrame, target: str = "rhr"
...
def evaluate_model(
    model: nn.Module, test_loader: DataLoader, scaler_y: StandardScaler
) -> Dict:
...
def create_optuna_objective(
    X_train, y_train, X_val, y_val, input_dim: int, architecture: str = "transformer"
):
...
def run_mega_brutal_experiment():
    ...
    X_train_scaled = scaler_X.fit_transform(X_train.reshape(-1, input_dim)).reshape(
        X_train.shape
    )
    X_val_scaled = scaler_X.transform(X_val.reshape(-1, input_dim)).reshape(X_val.shape)
    X_test_scaled = scaler_X.transform(X_test.reshape(-1, input_dim)).reshape(
        X_test.shape
    )
    ...
    architectures = {
        "transformer": lambda: TransformerHealthPredictor(
            input_dim, d_model=128, nhead=8, num_layers=4
        ),
        "tft": lambda: TemporalFusionTransformer(
            input_dim, hidden_dim=128, num_heads=4, num_layers=3
        ),
        ...
        "uncertainty_aware": lambda: UncertaintyAwarePredictor(
    ...
    for arch in optuna_architectures:
        ...
        objective = create_optuna_objective(
            X_train_scaled, y_train_scaled, X_val_scaled, y_val_scaled, input_dim, arch
        )
    ...
    for arch in optuna_architectures:
        ...
        if arch == "transformer":
            ...
            model = CNNLSTMHybrid(
                input_dim,
                best_params["cnn_channels"],
                best_params["lstm_hidden"],
                best_params["num_lstm_layers"],
                best_params["dropout"],
            )
    ...
    results["ensemble_results"] = {
        "metrics": metrics,
        "learned_weights": {
            arch: float(w) for arch, w in zip(optuna_architectures, learned_weights)
        },
    }
    ...
    for persona_name in PERSONAS:
        ...
        persona_dataset = TensorDataset(
            torch.FloatTensor(X_persona), torch.FloatTensor(y_persona)
        )
        ...
        print(
            f"  {persona_name.name}: MAE = {metrics['mae']:.4f} bpm, R² = {metrics['r2']:.4f}"
        )
    ...
    model_mae_list = [
        (name, res["metrics"]["mae"]) for name, res in results["model_results"].items()
    ]
    ...
    ensemble.eval()
    best_model = optimized_models.get(
        best_single.replace("_optimized", ""), trained_models.get(best_single)
    )
    ...
    ensemble_preds = scaler_y.inverse_transform(
        np.array(ensemble_preds).reshape(-1, 1)
    ).flatten()
    best_preds = scaler_y.inverse_transform(
    ...
    results["statistical_tests"] = {
        "ensemble_vs_best_single": {
            ...
            "improvement": float(
                (best_errors.mean() - ensemble_errors.mean()) / best_errors.mean() * 100
            ),
        }
    }
    ...
    print(
        f"  Improvement: {(best_errors.mean() - ensemble_errors.mean()) / best_errors.mean() * 100:.2f}%"
    )
    ...
    all_results.append(
        (
            "deep_ensemble",
            {"metrics": results["ensemble_results"]["metrics"], "parameters": "N/A"},
        )
    )
    ...
    for name, data in all_results:
        ...
        print(
            f"{name:<30} {mae:>10.4f} {rmse:>10.4f} {r2:>10.4f} {params_str:>12}{marker}"
        )
