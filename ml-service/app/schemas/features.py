...
class NutritionFeatures(BaseModel):
    ...
    # Macro ratios
    protein_ratio: float = Field(
        ..., ge=0, le=1, description="Protein calories / total calories"
    )
    carbs_ratio: float = Field(
        ..., ge=0, le=1, description="Carbs calories / total calories"
    )
    fat_ratio: float = Field(
        ..., ge=0, le=1, description="Fat calories / total calories"
    )

    # Meal timing
    first_meal_time: Optional[float] = Field(
        None, description="Time of first meal (hour of day, 0-24)"
    )
    last_meal_time: Optional[float] = Field(
        None, description="Time of last meal (hour of day, 0-24)"
    )
    eating_window: Optional[float] = Field(
        None, description="Hours between first and last meal"
    )
    ...
    # Regularity
    meal_regularity: float = Field(
        ..., ge=0, le=1, description="Consistency of meal timing (0-1)"
    )

    # Calorie balance (requires user profile with TDEE)
    calorie_deficit: Optional[float] = Field(
        None, description="Calories below TDEE (negative = surplus)"
    )
    calorie_deficit_7d: Optional[float] = Field(
        None, description="7-day average calorie deficit"
    )
...
class ActivityFeatures(BaseModel):
    ...
    calories_burned_daily: float = Field(
        ..., description="Calories burned from activity"
    )

    # Rolling averages (7 days)
    steps_7d_avg: float = Field(..., description="7-day average steps")
    active_minutes_7d_avg: float = Field(
        ..., description="7-day average active minutes"
    )
    calories_burned_7d_avg: float = Field(
        ..., description="7-day average calories burned"
    )
    ...
    high_intensity_minutes: float = Field(
        ..., description="High-intensity workout minutes today"
    )

    # Recovery
    recovery_time: float = Field(
        ..., description="Hours since last high-intensity workout"
    )
    days_since_rest: int = Field(
        ..., description="Days since last rest day (no workouts)"
    )

    # Activity type distribution (7 days)
    cardio_minutes_7d: float = Field(..., description="Cardio minutes in last 7 days")
    strength_minutes_7d: float = Field(
        ..., description="Strength training minutes in last 7 days"
    )
    flexibility_minutes_7d: float = Field(
        ..., description="Flexibility/yoga minutes in last 7 days"
    )
...
class HealthFeatures(BaseModel):
    ...
    rhr_7d_std: Optional[float] = Field(
        None, description="7-day RHR standard deviation"
    )
    rhr_trend: Optional[float] = Field(
        None, description="RHR trend: slope of last 7 days"
    )
    rhr_baseline: Optional[float] = Field(
        None, description="User's 30-day RHR baseline"
    )
    rhr_deviation: Optional[float] = Field(
        None, description="Current RHR deviation from baseline"
    )
    ...
    hrv_7d_std: Optional[float] = Field(
        None, description="7-day HRV standard deviation"
    )
    hrv_trend: Optional[float] = Field(
        None, description="HRV trend: slope of last 7 days"
    )
    hrv_baseline: Optional[float] = Field(
        None, description="User's 30-day HRV baseline"
    )
    hrv_deviation: Optional[float] = Field(
        None, description="Current HRV deviation from baseline"
    )

    # Sleep
    sleep_duration_last: Optional[float] = Field(
        None, description="Sleep duration last night (hours)"
    )
    sleep_quality_last: Optional[float] = Field(
        None, ge=0, le=100, description="Sleep quality last night (0-100)"
    )
    sleep_duration_7d_avg: Optional[float] = Field(
        None, description="7-day average sleep duration"
    )
    ...
    recovery_score_7d_avg: Optional[float] = Field(
        None, description="7-day average recovery score"
    )
...
class TemporalFeatures(BaseModel):
    ...
    menstrual_cycle_day: Optional[int] = Field(
        None, description="Day of menstrual cycle (if tracked)"
    )
    menstrual_cycle_phase: Optional[str] = Field(
        None, description="Cycle phase: follicular/ovulation/luteal"
    )
...
class InteractionFeatures(BaseModel):
    ...
    protein_per_kg: Optional[float] = Field(
        None, description="Protein (g) per kg body weight"
    )
    calories_per_kg: Optional[float] = Field(
        None, description="Calories per kg body weight"
    )

    # Nutrition per activity
    carbs_per_active_minute: Optional[float] = Field(
        None, description="Carbs per active minute"
    )
    protein_per_workout: Optional[float] = Field(
        None, description="Protein per workout"
    )

    # Recovery-adjusted nutrition
    protein_to_recovery: Optional[float] = Field(
        None, description="Protein intake / recovery time"
    )
    carbs_to_intensity: Optional[float] = Field(
        None, description="Carbs / workout intensity"
    )
...
class EngineerFeaturesRequest(BaseModel):
    ...
    categories: list[FeatureCategory] = Field(
        default=[FeatureCategory.ALL], description="Categories of features to engineer"
    )
    lookback_days: int = Field(
        default=30, ge=7, le=90, description="Days of historical data to use (7-90)"
    )
    force_recompute: bool = Field(
        default=False, description="Force recomputation even if cached"
    )
...
class EngineerFeaturesResponse(BaseModel):
    ...
    missing_features: int = Field(
        ..., description="Number of features with missing data"
    )
...
class FeaturesResponse(BaseModel):
    ...
    features: Dict[str, float] = Field(
        ..., description="Flat dictionary of all features"
    )
