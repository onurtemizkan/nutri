...
import { WeightUnit, kgToLb, lbToKg, WeightRecord } from '@/lib/types/weight';
...
export default function EditWeightScreen() {
  ...
  const loadRecord = async () => {
    ...
    try {
      const data = await weightApi.getById(id);
      setRecord(data);
      setWeightInput(data.weight.toFixed(1));
      setRecordedAt(new Date(data.recordedAt));
    } catch (error) {
      showAlert('Error', getErrorMessage(error, 'Failed to load weight record'));
      router.back();
    } finally {
      setIsLoading(false);
    }
  ...
  const handleSave = async () => {
    ...
    try {
      await weightApi.update(id, {
        weight: weightInKg,
        recordedAt: recordedAt.toISOString(),
      });
      router.back();
    } catch (error) {
      showAlert('Error', getErrorMessage(error, 'Failed to update weight record'));
    } finally {
  ...
  const handleDelete = () => {
    ...
    showAlert('Delete Weight Record', 'Are you sure you want to delete this weight record?', [
      { text: 'Cancel', style: 'cancel' },
      {
        ...
        onPress: async () => {
          ...
          try {
            ...
            showAlert('Error', getErrorMessage(error, 'Failed to delete weight record'));
            setIsSubmitting(false);
          }
        },
      },
